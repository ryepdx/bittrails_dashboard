...
def get_args():
    parser = argparse.ArgumentParser(description='Run Selenium tests on Dashboard.')
    parser.add_argument('--port', '-dp', action='store', dest='port', type=int,
        default=5001, help='the port Dashboard is running on')
    parser.add_argument('--host', '-dh', action='store', dest='host', type=str,
        default='dashboard', help='the hostname Dashboard is running on')
